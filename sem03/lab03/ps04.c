 ...
 * Course CSE2153
 * Problem Set: 4
 * Problem: Write a program in C to insert a new node at the beginning of a
 * Singly Linked List.
 */
#include <stddef.h>
#include <stdio.h>
...
void prepend(struct Node **head, int value) {
  // create a leading node
  struct Node *node = create_node(value);
  //  set the next of the new node to the old head;
  node->next = (*head);
  // finally replace the head with the new one
  (*head) = node;
}
/**
